...
// Limit is a pluggable function to enforce limits on the object. This wraps the default implementation to
// always return success so that all defaults will be applied. Validation will occur after the overrides.
...
func internalSupportsAttributes(attr admission.Attributes) bool {
	return attr.GetResource() == kapi.Resource("pods") && attr.GetSubresource() == ""
}
